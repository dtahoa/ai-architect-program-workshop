...
flowchart LR
  subgraph Client["Mobile / Web client (Capacitor + PWA)"]
    UI["Tutor Chat UI[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]Math keyboard (LaTeX-lite)[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]SSE + offline queue"]
  end
  subgraph Edge["Edge"]
    CDN["CDN / WAF / Rate limit"]
    GW["API Gateway[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]AuthN (JWT), consent check, per-tenant quota"]
  end
  subgraph Core["Tutor Plane (Kubernetes)"]
    ORCH["Tutor Orchestrator[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]Socratic Dialogue Policy (state machine)[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]Session state in Redis"]
    IG["Input Guard[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]PII redact · jailbreak · off-topic · safety"]
    OG["Output Guard[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]Answer-Leak Detector (CAS+classifier) · toxicity · off-topic"]
    RET["Hint Retriever[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]Hint Ladder + misconception bank (pgvector, Redis cache)"]
    GRD["Grader Service[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]SymPy CAS equivalence · step alignment"]
    LM["Learner Model Service[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]BKT per skill · pseudonymous"]
    PG["Practice Generator (async)[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]Template + LLM T2 + CAS verify + teacher QA"]
    MG["Model Gateway (LiteLLM)[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]tiering · fallback · prompt cache · budget · prompt registry"]
  end
  subgraph Data["Data Plane"]
    REDIS[("Redis[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]session · cache")]
    PGDB[("PostgreSQL + pgvector[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]content, hint ladder, mastery")]
    KAFKA[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:document-link:%22Kafka%3Cbr%2F%3Etutor%20events%22]]
    LAKE[("Lakehouse (Iceberg/S3)[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]events · eval · lineage")]
    PII[("Identity & Consent Store[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]](in-country, encrypted, separate KMS)")]
  end
  subgraph Offline["Offline / Enrichment Plane"]
    ENR["Content Enrichment Pipeline[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]Hint Ladder gen (LLM T3) → teacher review"]
    SYN["Synthetic Dialogue Factory[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]teacher-student simulation → curation"]
    EVAL["Eval Harness[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]golden set · red-team · LLM-judge · slices"]
    FT["Fine-tune Pipeline (Phase 2)[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]SFT/DPO tier-1 · registry"]
  end
  subgraph Obs["Observability"]
    O11Y["Traces (OTel) · metrics · LLM traces (Langfuse)[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]eval-in-prod sampler · cost meter"]
  end
  UI --> CDN --> GW --> ORCH
  ORCH --> IG --> ORCH
  ORCH --> RET --> PGDB
  ORCH --> GRD
  ORCH --> LM --> PGDB
  ORCH --> MG --> LLMP["LLM Providers[[ORCA_RICH_MD:dbb9738890eb6ddd30472959621442f1:inline-html:%3Cbr%2F%3E]]T1 / T2 (+ vLLM self-host option)"]
  MG --> OG --> ORCH
  ORCH --> REDIS
  ORCH --> KAFKA --> LAKE
  KAFKA --> LM
...
5. **Dialogue Policy** (deterministic) updates state: correct → next step; 1st wrong → keep hint\_level, ask for an explanation of the error; 2nd wrong / stuck signal ("I don't know", idle &gt; 45s) → raise hint\_level; hint\_level = max &amp; attempts ≥ N → `REVEAL_ALLOWED` (only now is the answer put in the prompt, with a request that the student re-solve a similar problem).
6. **Prompt assembly**: system prompt (version `sp-v3.2`), persona by grade band, **only hints ≤ hint\_level**, misconception explanation, last 6 turns, mastery hints (e.g. "weak on fractions → slow down"). **No final answer** unless `REVEAL_ALLOWED`.
...
- **Options:** (A) One best model for every turn. (B) **Tiering by difficulty signals** (stuck\_signals, problem type, guard-retry) through a gateway. (C) Self-host everything from day 1.
...
- **Source registry:** table `content_source` (id, owner, license, grade, subject, version, review\_status); only `review_status = approved` enters retrieval.
...
pgvector, **HNSW** index (m=16, ef\_construction=200, ef\_search=64) on `concept_card` and `misconception`; partitioned by `subject`; &lt; 1M vectors, p95 &lt; 20ms. Scaling plan: read replicas → partition by grade → Qdrant when &gt; 10M vectors or complex filtering is needed at p95 &lt; 20ms.
...
| --------------------------- | --------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| `content.problem.v2` (Avro) | problem\_id, grade, subject, skills\[\], statement\_latex, final\_answer (encrypted col), worked\_solution\_steps\[\] | schema registry, CAS parses ≥ 99%    |
| `behavior.attempt.v1`       | learner\_id (pseudonymous), problem\_id, correct, duration\_ms, abandoned\_at\_step                                   | Great Expectations: null rate, drift |
| `tutor.turn.v1`             | session\_id, turn\_no, state\_before/after, guard\_verdicts, model\_id, prompt\_version, tokens, cost, latency        | producer-side validation             |
| `consent.status.v1`         | learner\_id, parent\_verified, scopes\[\], expires\_at                                                                | freshness &lt; 5 minutes             |
...
- **Parental consent:** consent ledger (scopes: tutor, retain\_chat\_30d, analytics); missing scope → session not opened. Right to erasure: deleting the `learner_id` mapping leaves the remaining data anonymous; conversations deleted by 30-day TTL; DSAR SLA 15 days.
...
| ------------------------------------------------------------------------------------ | ------------------- | --------------------------------------------------------------------------------- |
| Grade 6/7/8/9                                                                        | grade\_band         | Pedagogy ≥ 3.8 per grade                                                          |
...
| **Retrieval quality**                                          | Ladder hit@1 by problem\_id = 100%; misconception recall@3 ≥ 0.85 on G-Grade                                                                                                                                                                                      |
...
| **Inputs**                 | `system_prompt@version`; `persona(grade_band)`; `ladder[0..hint_level]` (questions + expected steps, **no final answer**); `misconception_hint?`; `mastery_hints[]` (3 flags); `history[-6:]` (PII redacted); `student_msg` (text/LaTeX, ≤ 2k tokens); `state` (attempts, reveal\_allowed). Format: chat messages + JSON state block                                    |
...
| **Dependencies**           | Hint Ladder (ladder\_version), Input/Output Guard classifiers (guard\_version), Grader (SymPy), Learner Model Service, Model Gateway, Redis session                                                                                                                                                                                                                     |
...
| L-208 | 5 consecutive wrong attempts with the same random value   | No reveal (distinct\_wrong &lt; 2); ask the student to explain their method |
