# Bản tổng hợp Workshop 1 — Variant B: AI-Retrofit

## 1. Workshop thực sự kiểm tra điều gì?

Workshop không đánh giá nhóm dựa trên việc sơ đồ kiến trúc có đẹp hay sử dụng nhiều công nghệ hay không. Trainer muốn kiểm tra khả năng đi xuyên suốt chuỗi quyết định:

> **Requirement → Constraint → Architecture → Evidence → Trade-off → Risk → Decision defence**

Nhóm phải chứng minh rằng:

* Đã hiểu đúng bài toán kinh doanh.
* Phân biệt được yêu cầu bắt buộc và điều có thể thương lượng.
* Chọn kiến trúc đơn giản nhất nhưng vẫn đáp ứng NFR.
* Có phép tính capacity, thời gian và chi phí.
* Biết hệ thống sẽ làm gì khi AI lỗi.
* Nhận diện được giới hạn của dữ liệu.
* Không sử dụng AI hoặc LLM chỉ để làm kiến trúc trông hiện đại.
* Có thể giải thích tại sao chọn phương án này và loại phương án khác.

Đề bài yêu cầu tối đa 8 slide, trình bày 7 phút, Q&A 4 phút và 1 phút chuyển nhóm. Trainer đã biết scenario, vì vậy nhóm không cần kể lại toàn bộ bối cảnh NovaMart mà phải đi thẳng vào các quyết định kiến trúc. 

---

# 2. Thông điệp kiến trúc trung tâm

## Quyết định chính

> **Keep MERLIN in control; let AI fail without stopping a truck.**

Kiến trúc sử dụng một:

> **External, asynchronous, fail-open AI sidecar**

AI được đặt **bên cạnh MERLIN**, không được đặt bên trong MERLIN và không trở thành dependency bắt buộc của chuỗi tạo đơn hàng.

AI sidecar thực hiện:

1. Kiểm tra dữ liệu snapshot.
2. Tạo demand forecast bằng statistical ML.
3. Tạo confidence interval hoặc forecast quantile.
4. Áp dụng deterministic replenishment constraints.
5. Ghi các recommendation hợp lệ vào override table thông qua certified adapter.

MERLIN vẫn:

* Là system of record.
* Tự chạy min/max replenishment engine.
* Tự tạo official supplier order.
* Tự quản lý review queue.
* Là hệ thống duy nhất gửi supplier EDI.

Nếu AI không có output hợp lệ, MERLIN tiếp tục chạy logic min/max hiện tại. Vì vậy, bỏ toàn bộ sidecar khỏi hệ thống thì các cửa hàng vẫn nhận được hàng vào buổi sáng. 

---

# 3. Requirement — Bài toán phải giải quyết

## 3.1 Business requirements

NovaMart muốn cải thiện các chỉ số:

| Chỉ số                      |          Hiện tại |                     Mục tiêu |
| --------------------------- | ----------------: | ---------------------------: |
| Out-of-stock rate           |              7.2% |                         3.0% |
| Fresh waste                 |              4.8% |                         3.0% |
| Demand forecast WAPE        |               41% |                         ≤25% |
| Thời gian nhân viên tra cứu | Khoảng 50 phút/ca |                  <15 phút/ca |
| Store associates            |             8.400 | Giữ nguyên, không giảm người |

Đây không phải chương trình giảm nhân sự. Kiến trúc không được tạo business case bằng cách loại bỏ store associates. 

## 3.2 Target capabilities

Đề bài đưa ra sáu capability:

1. Demand forecasting theo SKU, store và ngày.
2. Automated replenishment.
3. Fresh markdown hoặc dynamic pricing.
4. Store associate copilot.
5. Online order substitution.
6. Promotion planning support.

Tuy nhiên, nhóm không bắt buộc phải tự động hóa toàn bộ ngay Release 1. Một quyết định kiến trúc hợp lệ là trì hoãn capability nếu chưa đủ interface, dữ liệu, bằng chứng an toàn hoặc capacity.

Trong thiết kế hiện tại:

* Forecasting và replenishment recommendation là trọng tâm AI-Retrofit.
* Associate copilot được tách hoàn toàn khỏi POS và ordering.
* Automated price write và online substitution AI chưa được bật trong Release 1.
* Markdown chỉ được phân tích offline và phải có audit đầy đủ. 

---

# 4. Constraint analysis — Những ràng buộc quyết định kiến trúc

## 4.1 MERLIN là authority không được thay thế

MERLIN là SAP ERP on-premises được tùy chỉnh từ năm 2006. Nó sở hữu:
...
MERLIN chỉ phát hành hai lần mỗi năm thông qua SAP-certified partner, không có automated test suite và core replacement đã bị loại khỏi phạm vi sau thất bại $22M năm 2021. 

Điều này dẫn tới quyết định:

> Không sửa MERLIN core và không tạo một hệ thống order master song song.

## 4.2 Chỉ có ba extension point hợp lệ

AI chỉ được tương tác với MERLIN thông qua:

1. **Nightly batch/snapshot** — đầu vào read-only.
2. **Override table** — MERLIN đọc trước khi tự tạo order.
3. **Post-generation review queue** — category manager review sau khi MERLIN tạo order.

Không được tự tạo thêm interface thứ tư. 

## 4.3 POS hoàn toàn không được đụng tới

POS có khoảng 7.000 lanes, chạy offline-first và chỉ đồng bộ nightly. Thay đổi POS cần chín tháng re-certification với payment processor.

Do đó:

* AI không gọi POS.
* POS không gọi AI.
* AI không có POS credential.
* POS không phụ thuộc runtime vào sidecar.
* Store network mất kết nối không được ảnh hưởng đến revenue path.

## 4.4 Supplier EDI và thời hạn 04:00 bất biến

EDI có fixed schema và 2.300 suppliers đang tích hợp với schema hiện tại.

Order phải hoàn tất trước 04:00 để distribution centre chuẩn bị cho xe rời đi lúc 06:00. Nếu trễ, cửa hàng không nhận hàng trong ngày và không có rerun.

Vì vậy:

* Sidecar không gửi EDI.
* Sidecar không gọi trực tiếp supplier.
* Không thay đổi EDI schema.
* Không chạy một “late recovery” sau khi cửa sổ an toàn đã đóng.

## 4.5 Inventory chỉ chính xác 78%

Điều này có nghĩa hơn một trong năm inventory record có thể không phản ánh đúng hàng thực tế trên kệ.

Với khoảng 7,04 triệu store–SKU stocked pairs:

[7.04M \times 22% \approx 1.5488M]

Khoảng 1,55 triệu pair có khả năng sai.

Do đó inventory không được coi là một số tuyệt đối. Nó phải được xử lý như một biến không chắc chắn gồm:

* Snapshot age.
* Lower và upper inventory bounds.
* Calibrated confidence interval.
* Interval coverage.
* Absolute quantity caps.
* Shelf-life constraints.
* Suppression khi độ tin cậy thấp.

Thiết kế không gọi POS để cố lấy “real-time shelf truth”, vì dữ liệu đó không tồn tại theo constraint của bài toán. 

## 4.6 Data residency và audit

Một trong ba thị trường không cho phép loyalty personal data ra khỏi quốc gia.

Release 1 vì vậy:

* Không export member-level loyalty data.
* Country-local deployment đối với personal data.
* DLP và egress deny.
* Mọi AI-influenced decision phải có trace.
* POS retention hiện tại ít nhất năm năm không bị AI thay đổi. 
...
> **External asynchronous fail-open sidecar**

“External” nghĩa là AI chạy ngoài MERLIN.

“Asynchronous” nghĩa là MERLIN không chờ response trực tiếp từ AI trong lúc tạo order.

“Fail-open” trong bối cảnh này nghĩa là:

> AI lỗi thì AI influence bị bỏ qua, nhưng incumbent replenishment của MERLIN vẫn tiếp tục.

Không nên hiểu fail-open là cho phép output AI không hợp lệ đi qua.

## 5.2 Các thành phần bên trong sidecar
...
Feature preparation
    ↓
Statistical forecasting ML
    ↓
Forecast quantiles / confidence
    ↓
Deterministic optimizer
    ↓
Certified MERLIN adapter
    ↓
Override table
    ↓
MERLIN native order generation
    ↓
Review queue
    ↓
MERLIN EDI
...
Các container chính gồm:

* **Snapshot validation:** kiểm tra freshness, completeness, schema, checksum và residency.
* **Statistical forecast ML:** tạo demand forecast và uncertainty.
* **Deterministic optimizer:** áp dụng case pack, shelf life, DC capacity, inventory interval và category caps.
* **Certified adapter:** chỉ ghi bounded, current-run rows.
* **Disable control:** epoch, short-lived permit và independent revoke.
* **Append-only audit:** lưu model version, ruleset, input manifest, recommendation và publication evidence.

Associate copilot là một boundary riêng, không kết nối POS hoặc ordering.  

## 5.3 Authority boundary

AI tạo ra **recommendation**, không tạo official order.

MERLIN đọc recommendation trong override table rồi vẫn tự thực hiện native generation:

```text
AI recommendation
    ≠ supplier order

MERLIN output after native generation
    = official supplier order
```

Số lượng order thực tế của MERLIN chỉ có thể được trả lại sidecar sau khi generation thông qua review queue và certified correlation fields.

Nếu review queue không cung cấp contract chính thức cho order ID, quantity và correlation key, automated influence phải tiếp tục bị disable. Không được đọc undocumented table để lấy số lượng. 

---

# 6. Chứng minh bằng số liệu

## 6.1 Deadline trước 04:00

Thiết kế chia nightly window như sau:

| Thời gian   | Hoạt động                        |
| ----------- | -------------------------------- |
| 22:00–22:20 | Validate immutable snapshot      |
| 22:20–00:20 | Statistical forecasting          |
| 00:20–01:20 | Deterministic optimization       |
| 01:20–01:30 | Final bounded retry              |
| 01:30–01:45 | Reconcile và complete manifest   |
| 01:45–02:00 | Certified publish                |
| 02:00–04:00 | MERLIN generation, review và EDI |

Các hard gates:

* **01:30:** không bắt đầu retry mới.
* **01:45:** tất cả required partitions phải đầy đủ.
* **02:00:** đóng toàn bộ AI writes.
* **Sau 02:00:** MERLIN sở hữu phần còn lại của order window.

AI target path là khoảng **198,7 phút** trong cửa sổ 240 phút từ 22:00 đến 02:00, để lại khoảng **41,3 phút headroom**.

Tuy nhiên đây là design target, chưa phải benchmark result. Mốc 02:00 chỉ hợp lệ nếu đo được MERLIN P99 không quá 90 phút và vẫn còn tối thiểu 30 phút dự phòng trước 04:00. Nếu MERLIN cần nhiều thời gian hơn, AI write-close phải chuyển sớm hơn. 

## 6.2 AI chết lúc 03:30

Nếu sidecar phục hồi lúc 03:30:

* Không chạy lại forecast.
* Không ghi override.
* Không thay đổi order của current run.
* Chỉ lưu kết quả để diagnosis và cải thiện lần chạy sau.
* MERLIN tiếp tục generation, review và EDI hiện tại.

Đây là một quyết định an toàn có chủ đích: **late correct output vẫn không còn hợp lệ cho current run**. 

## 6.3 Normal và six-times peak capacity

Normal volume:

* Khoảng 27 triệu forecasts mỗi đêm.
* Khoảng 7,04 triệu stocked candidates cần deterministic optimization.

Six-times Lunar New Year test:

* 162 triệu forecasts.
* 42,24 triệu optimizations.

Target throughput:

* Forecast: ít nhất 30.000 records/second.
* Optimizer: ít nhất 15.000 records/second.

Phép tính peak:

[162M / 30,000 = 5,400\ seconds = 90\ minutes]

[42.24M / 15,000 = 2,816\ seconds \approx 46.93\ minutes]

Critical minimum:

* Forecast dưới 22.500/second là critical.
* Optimizer dưới 11.733/second là critical.
* Phải vượt qua ba lần six-times test liên tiếp trước khi production influence được phép. 

## 6.4 Storage

Giả định mỗi forecast output khoảng 1,2 KB:

Normal:

[27M \times 1.2KB = 32.4GB/run]

Six-times peak:

[162M \times 1.2KB = 194.4GB/run]

Đây vẫn là planning assumption. Actual bytes/output và retention tier phải được đo trước production.
...
Cost ceiling cho mỗi nominal forecast:

[$4M / 9.855B \approx $0.0004059]

Internal target $3,8M/year:

[$3.8M / 9.855B \approx $0.0003856]

Vì vậy, bất kỳ model nào có blended cost cao đáng kể hơn khoảng $0,0004 mỗi forecast sẽ không phù hợp.

Cost phải bao gồm toàn bộ AI run cost, không chỉ inference:
...
Nếu annualized cost vượt $4M hoặc business case dựa trên cắt giảm 8.400 associates thì production expansion phải dừng. 

---

# 7. Technology selection và lý do lựa chọn

## 7.1 Integration pattern

| Phương án                     | Weighted score |
| ----------------------------- | -------------: |
| External asynchronous sidecar |       **4.80** |
| Event-driven boundary         |           2.95 |
| Synchronous API gateway       |           1.80 |
| In-process MERLIN plugin      |           1.30 |

Sidecar thắng vì đáp ứng tốt nhất:

* Legacy safety.
* Fail-open continuity.
* Supported-interface fit.
* Rollback.
* Isolation khỏi MERLIN.

## 7.2 Execution platform

| Phương án                 | Weighted score |
| ------------------------- | -------------: |
| Managed elastic batch     |       **4.65** |
| Kubernetes batch workers  |           4.15 |
| Managed Spark             |           3.90 |
| Fixed on-premises compute |           3.10 |

Managed elastic batch được chọn vì:

* Có thể scale cho six-times peak.
* Phù hợp nightly workload.
* Có deadline-aware orchestration.
* Không cần duy trì fixed peak infrastructure quanh năm.

Lựa chọn provider cụ thể vẫn phải qua residency, benchmark và cost evidence.

## 7.3 Forecast model strategy

| Phương án                          | Weighted score |
| ---------------------------------- | -------------: |
| Hybrid hierarchical statistical ML |       **4.55** |
| One global statistical model       |           4.35 |
| Classical baseline                 |           3.50 |
| Per-store-SKU model                |           3.20 |

Hybrid model được ưu tiên vì:

* Chia sẻ signal giữa sparse series.
* Xử lý promotion tốt hơn.
* Hỗ trợ uncertainty interval.
* Không cần duy trì 27 triệu model riêng biệt.

Tuy nhiên sensitivity analysis cho thấy khi chuyển trọng số từ WAPE sang cost, hybrid và global model cùng đạt 4.45. Vì vậy, model cuối cùng phải được quyết định bằng replay evidence, không phải chỉ dựa trên scoring matrix. 

---

# 8. Vì sao không dùng các kiến trúc khác?

## Synchronous API gateway

Không chọn vì MERLIN sẽ phải chờ AI trong critical path.
...
thì MERLIN generation có thể bị chặn, vi phạm yêu cầu “AI must never break the ordering chain”.

## In-process MERLIN plugin
...
* Failure của AI có thể trở thành failure của MERLIN.
* Không phù hợp timeline tám tháng.

## Event-driven integration xuyên legacy boundary

Không chọn làm primary pattern vì MERLIN không cung cấp supported real-time event interface. Tự đọc scheduler hooks, undocumented tables hoặc database log sẽ tạo interface thứ tư không được phép.

Internal event hoặc queue có thể được dùng bên trong sidecar, nhưng không được biến MERLIN thành runtime event dependency.

## Core replacement

Không nằm trong phạm vi:

* Đã có chương trình replacement thất bại $22M.
* Timeline chỉ tám tháng.
* Board không chấp nhận gradual replacement.
* Replacement không cần thiết để đạt mục tiêu hiện tại.

---

# 9. Trade-off thực sự được chấp nhận

Chọn sidecar không có nghĩa là kiến trúc đơn giản hơn về mọi mặt.

Nhóm chấp nhận:

* Duplicate snapshot.
* Data reconciliation giữa hai hệ thống.
* SAP-certified adapter work.
* Hai hệ thống monitoring và traceability.
* Provider dependency.
* Data-transfer và residency control.
* Chi phí vận hành sidecar riêng.
* Giá trị ban đầu bị giới hạn do bounded influence.
* Không có real-time inventory.
* Một số capability bị trì hoãn.

Đổi lại, nhóm nhận được:

* MERLIN không phụ thuộc AI.
* AI có thể scale và release độc lập.
* Rollback không cần rollback MERLIN.
* Có thể shadow mà không ghi dữ liệu.
* Có kill switch độc lập.
* AI lỗi không làm dừng ordering chain.

Đây là trade-off thật vì nhóm phải trả thêm chi phí và complexity để mua được isolation và safety.

---

# 10. Rủi ro chính và cách kiểm soát

| Rủi ro                   | Điểm | Critical trigger                                               | Safe response                       |
| ------------------------ | ---: | -------------------------------------------------------------- | ----------------------------------- |
| Miss deadline            |   20 | Reserve <30 phút, write ≥02:00 hoặc EDI ≥04:00                 | Bypass AI                           |
| Inventory uncertainty    |   20 | Interval coverage <85%, missing interval hoặc data age >24 giờ | Suppress row/scope                  |
| Peak/model failure       |   16 | WAPE >30%, forecast <22.500/s hoặc optimizer <11.733/s         | Disable model/slice                 |
| Legacy adapter integrity |   15 | Stale commit, thay đổi manual row hoặc MERLIN wait >0          | Kill adapter, revoke access, shadow |
| Food-safety/legal        |   15 | Generated/unsupported/wrong-SKU safety output                  | Refuse, disable và escalate         |

Các risk khác như residency, cost overrun, review fatigue và provider outage vẫn được kiểm soát trong NFR nhưng có score thấp hơn top five hiện tại. 

---

# 11. LLM được sử dụng ở đâu và không được sử dụng ở đâu?
...
* Pricing decision.
* Food-safety hoặc allergen display.
* Logic quyết định supplier order.

Lý do:

* Không phải language problem.
* Khó bảo đảm deterministic reproducibility.
* Chi phí không phù hợp 27 triệu forecasts/ngày.
* Có hallucination risk.
* Khó audit numeric decision.
* Food-safety có zero-error expectation.

## Có thể dùng LLM giới hạn cho

* Intent classification.
* Entity extraction.
* Non-safety question answering có citation.
* Hỗ trợ ngôn ngữ cho nội dung không liên quan safety.

Đối với allergen hoặc food safety, hệ thống chỉ được:

* Lấy exact approved evidence.
* Kiểm tra SKU, country, locale và version.
* Hiển thị fixed approved wording.
* Gắn citation.
* Refuse nếu evidence thiếu, cũ, conflict hoặc không có signature.

Không được dùng LLM để paraphrase, summarize hoặc translate safety content. 

---

# 12. Kill switch

Kill switch phải là một cơ chế kỹ thuật có thể kiểm thử, không phải chỉ là một nút trên UI.

Quy trình:

1. Duty Manager tạo một `DISABLING epoch`.
2. Dừng phát hành short-lived permits.
3. Adapter kiểm tra lại epoch bên trong transaction.
4. Reject hoặc rollback stale transactions.
5. Nếu adapter không phản hồi, SAP break-glass identity revoke DB/network access.
6. Terminate active sessions.
7. Cleanup chỉ những unconsumed AI-owned rows dựa trên exact key/hash ledger.
8. Không được xóa manual rows.
9. Xác nhận không còn permit, session, transaction hoặc unexplained AI row.

Mục tiêu:

* Acknowledge disable: ≤30 giây.
* Reject new write: ≤60 giây.
* `VERIFIED_SAFE`: ≤5 phút.

Trong toàn bộ quá trình, MERLIN vẫn chạy incumbent min/max, review và EDI. 

---

# 13. Rollout strategy

Kiến trúc không được chuyển thẳng từ design sang full production.
...
* MERLIN timing.
* Override-table contract.
* Queue correlation fields.
...
* Inventory interval calibration.
* Six-times capacity.
* Storage.
* Cost.
* Failure scenarios.
...
* Tối thiểu bốn tuần đại diện.
* Zero MERLIN writes.
* So sánh AI recommendation với MERLIN result.
* Không influence production order.

## Phase 4 — Bounded pilot

* 20 stores.
* Hai low-risk, non-fresh categories.
* Tối đa 5.000 rows hoặc measured adapter cap thấp hơn.
* Tối đa 100 reviews mỗi đêm.
* Tối đa hai case packs/store–SKU hoặc lower category cap.
...
* NFR gate pass.
* Cost được Finance approve.
* Six-times tests pass.
* Kill-switch drills pass.
* Adapter contract được certify.
* Operational owners và on-call coverage được tài trợ. 

---

# 14. Cách bảo vệ quyết định trước trainer

## “Vì sao chọn sidecar?”

Vì sidecar là phương án duy nhất trong các lựa chọn đã đánh giá vừa:

* Không sửa MERLIN.
* Chỉ dùng ba supported extension points.
* Cho phép scale độc lập.
* Không tạo synchronous dependency.
* Có thể shadow và rollback độc lập.
* Đạt weighted score 4.80.

## “Vì sao không chọn giải pháp đơn giản hơn?”

Synchronous API nhìn đơn giản hơn về diagram nhưng biến AI thành dependency của ordering chain. In-process plugin giảm số system nhưng vi phạm hard constraint và tăng blast radius.

Sidecar phức tạp hơn trong reconciliation và operations, nhưng complexity đó phục vụ một yêu cầu thật: **failure isolation**.

## “Có hoàn thành trước 04:00 không?”

Trên thiết kế:

* AI write đóng trước 02:00.
* AI target path 198,7 phút trong cửa sổ 240 phút.
* Forecast peak hoàn thành trong khoảng 90 phút ở 30.000/s.
* Optimization peak khoảng 46,93 phút ở 15.000/s.
* MERLIN có hai giờ cuối.

Nhưng đây chưa phải measured proof. Production chỉ được phép khi benchmark đạt ba lần six-times liên tiếp và MERLIN P99 ≤90 phút.

## “AI chết lúc 03:30 thì sao?”

Không làm gì với current run.

* Không retry.
* Không override.
* Không rerun.
* MERLIN tiếp tục.
* Output muộn chỉ được dùng cho diagnosis.

## “Inventory chỉ chính xác 78% thì xử lý thế nào?”

Không dùng inventory như một point truth.

* Dùng bounds và calibrated interval.
* Kiểm tra feasibility ở cả lower và upper bound.
* Áp dụng absolute caps.
* Suppress recommendation nếu interval không hợp lệ.
* Trả quyền quyết định về MERLIN.

## “Chi phí có vượt ngân sách không?”

Architecture target là $3,8M/năm với hard ceiling $4M/năm, tương đương khoảng $0,0004 mỗi nominal forecast.

Tuy nhiên production cần observed bill. Nếu annualized cost vượt ceiling thì freeze expansion và giảm các non-ordering workloads trước; không được hy sinh MERLIN continuity.

## “Có dùng LLM sai bài toán không?”

Không.

* Forecast dùng statistical ML.
* Quantity dùng deterministic optimizer.
* Food safety dùng exact deterministic rendering.
* LLM không được tạo numeric hoặc safety decision.

## “Có ảnh hưởng MERLIN, POS hoặc EDI không?”

* Không sửa MERLIN core.
* MERLIN không chờ AI.
* Không có AI route tới POS.
* Không có sidecar route tới supplier hoặc EDI.
* Chỉ certified adapter được ghi bounded rows vào supported override table.
* MERLIN vẫn tự tạo và gửi official order.

---

# 15. Kết luận kiến trúc

Kiến trúc không tuyên bố rằng AI luôn chính xác hoặc luôn sẵn sàng. Nó được xây dựng dựa trên giả định thực tế rằng:

* AI có thể lỗi.
* Data có thể stale.
* Inventory có thể sai.
* Batch có thể chậm.
* Network có thể mất.
* Provider có thể outage.
* Model có thể không đạt chất lượng.
* Operator có thể phải tắt hệ thống.

Điểm bảo vệ quan trọng nhất là:

> Khi bất kỳ điều gì không chắc chắn xảy ra, AI mất quyền influence trước khi MERLIN mất khả năng tạo order.

Do đó, thông điệp cuối cùng của workshop là:

> **Workshop đánh giá khả năng bảo vệ một kiến trúc bằng constraint, số liệu, trade-off, failure behavior và evidence — không chỉ khả năng vẽ một sơ đồ kiến trúc.**
